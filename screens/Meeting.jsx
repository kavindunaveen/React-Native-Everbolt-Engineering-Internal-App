...
export default function Example() {
  ...
  const createTable = () => {
    db.transaction(tx => {
      tx.executeSql(
        'CREATE TABLE IF NOT EXISTS booked_slots (id INTEGER PRIMARY KEY AUTOINCREMENT, date TEXT, slot TEXT, meeting TEXT)',
        [],
        (_, error) => {
          if (error) {
            console.error('Error creating booked_slots table:', error);
          } else {
            console.log('Table booked_slots created successfully.');
          }
        }
      );
    });
  ...
  const saveBookedSlot = (bookedSlot) => {
    db.transaction(tx => {
      tx.executeSql(
        'INSERT INTO booked_slots (date, slot, meeting) VALUES (?, ?, ?)',
        [bookedSlot.date, bookedSlot.slot, bookedSlot.meeting],
        (_, { insertId }) => {
          console.log('Booked slot inserted with ID:', insertId);
        },
        (_, error) => {
